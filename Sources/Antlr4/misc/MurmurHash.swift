...
public final class MurmurHash {
    ...
    public static func update2(_ hashIn: Int, _ value: Int) -> Int {
		...
		k = k.multipliedReportingOverflow(by: c2).0
        var hash = Int32(hashIn)
        hash = hash ^ k
        hash = (hash << r2) | (hash >>> (Int32(32) - r2))//hash = (hash << r2) | (hash >>> (32 - r2));
        (hash, _) = hash.multipliedReportingOverflow(by: m)
        (hash, _) = hash.addingReportingOverflow(n)
        //hash = hash * m + n;
    ...
    public static func finish(_ hashin: Int, _ numberOfWordsIn: Int) -> Int {
        ...
        hash = hash ^ numberOfWords.multipliedReportingOverflow(by: Int32(4)).0  //(numberOfWords * UInt32(4));
        hash = hash ^ (hash >>> Int32(16))   //hash = hash ^ (hash >>> 16);
        (hash, _) = hash.multipliedReportingOverflow(by: Int32(-2048144789))//hash * UInt32(0x85EBCA6B);
        hash = hash ^ (hash >>> Int32(13))//hash = hash ^ (hash >>> 13);
        //hash = UInt32(truncatingBitPattern: UInt64(hash) * UInt64(0xC2B2AE35)) ;
        (hash, _) = hash.multipliedReportingOverflow(by: Int32(-1028477387))
        hash = hash ^ (hash >>> Int32(16))//    hash = hash ^ (hash >>> 16);
